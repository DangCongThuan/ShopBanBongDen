...
<%--                        <th>Ngày chỉnh sửa</th>--%>
                      ...
                      <c:forEach items="${productList}" var="item">
                        <tr>
                          <td>
                            <img style="width: 150px; height: 100px" src="<c:url value="/get/image?fileName=${item.thumbnail}"/>">
                          </td>
                          <td>${item.id}</td>
                          <td>${item.name}</td>
                          <td>đ ${item.price}</td>
...
<%--                          <td>${item.modifiedDate}</td>--%>
                          ...
                          <td>
                            <a class="btn btn-success btn-action"
                               href="<c:url value="/admin/product/edit?id=${item.id}"/>"
                               data-toggle="tooltip" title="Chỉnh sửa"><i
                                    class="far fa-edit"></i></a>
                            <a class="btn btn-danger btn-action"
                               onclick="deletePro(this, '${item.id}')" data-toggle="tooltip"
...
<c:url var="urlDeleteProduct" value="/admin/product/delete"/>
<script>
  var cateTable = $('#productTable').DataTable({
    "language": {
